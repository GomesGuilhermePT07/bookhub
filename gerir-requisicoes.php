...
<body>
    ...
    <script>
        ...
        // Abrir modal quando clicar em "Entregar Livro ao Aluno"
        document.querySelectorAll('.entregar-livro').forEach(btn => {
            btn.addEventListener('click', function() {
                requisicaoId = this.dataset.id;
                livroIsbn = this.dataset.isbn;
                
                // Buscar detalhes do livro
                fetch(`assets/php/buscar_livro.php?isbn=${livroIsbn}`)
                    ...
                    .then(data => {
                        ...
                        document.getElementById('isbnLivro').textContent = livroIsbn;
                        
                        // Buscar capa do livro na Google Books API
                        fetch(`https://www.googleapis.com/books/v1/volumes?q=isbn:${livroIsbn}`)
                            .then(res => res.json())
                            .then(googleData => {
                                if (googleData.items && googleData.items[0].volumeInfo.imageLinks) {
                                    ...
                                    document.getElementById('capaLivro').src = 'assets/img/default-book.png';
                                }
                            })
                            .catch(() => {
                                document.getElementById('capaLivro').src = 'assets/img/default-book.png';
